...
function App() {
  ...
  useEffect(() => {
    ...
    console.log("agentKeyToUse", agentKeyToUse);
  ...
  const disconnectFromRealtime = () => {
    if (pcRef.current) {
      ...
      if (selectedAgentConfigSet && selectedAgentName === "stomachPainPatient") {
        // Create a summary of the conversation for the expert
        const conversationHistory = transcriptItems
          .filter(item => item.type === 'MESSAGE')
          .map(item => {
            // console.log("item", item);
            const role = item.role === "assistant" ? "Patient" : "Doctor";
      ...
      }, 50000); // Increased delay to give expert more time to respond
    }
  };
  ...
  const updateSession = (shouldTriggerResponse: boolean = false) => {
    ...
    if (selectedAgentName === "medicalExpert") {
    ...
    if (shouldTriggerResponse) {
      sendSimulatedUserMessage("Bataiye kaise aana hua, koi takleef hai?");
    }
  };
